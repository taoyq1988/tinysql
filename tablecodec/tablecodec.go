...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	rPrefix := key[9:11]
	_, handle, err = codec.DecodeInt(key[11:])
	if err != nil {
		return
	}
	if bytes.Compare([]byte{tPrefix}, tablePrefix) != 0 || bytes.Compare(rPrefix, recordPrefixSep) != 0 {
		err = errors.New("tablePrefix or recordPrefixSep invalid")
		return
	}
	return
}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	...
	indexPrefix := key[9:11]
	_, indexID, err = codec.DecodeInt(key[11:])
	if err != nil {
		return
	}
	if bytes.Compare([]byte{tPrefix}, tablePrefix) != 0 || bytes.Compare(indexPrefix, indexPrefixSep) != 0 {
		err = errors.New("tablePrefix or recordPrefixSep invalid")
		return
	}
